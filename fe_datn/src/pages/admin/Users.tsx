...
const Users = () => {
  ...
  const handleRoleChange = async (id: string, role: string) => {
    ...
    if (!role || !validRoles.includes(role)) {
      showNotification(
        "Vai trò không hợp lệ. Vui lòng chọn role hợp lệ.",
        "error",
      );
      console.log("Invalid role attempted:", role, "Valid:", validRoles);
      return;
    }
  ...
  const handleDeleteUser = async (id: string) => {
    ...
    try {
      ...
      console.error(err);
      showNotification(
        err.response?.data?.message || "Xoá người dùng thất bại",
        "error",
      );
    }
  };
